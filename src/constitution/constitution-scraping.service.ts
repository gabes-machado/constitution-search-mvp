...
import { RawConstitutionDataItem, ConstitutionIndexSchema } from './dto/constitution-data.dto';
...
export const CONSTITUTION_TYPESENSE_SCHEMA: CollectionCreateSchema = {
  ...
  fields: [
    ...
    { name: 'fullReference', type: 'string', optional: true, infix: true, sort: true },
    { name: 'text', type: 'string', infix: true },
    { name: 'hierarchicalTextContext', type: 'string', infix: true, optional: true },
    ...
    { name: 'parentArticleNumber', type: 'string', optional: true, facet: true },
    { name: 'sourceUrl', type: 'string' },
    { name: 'lastIndexedAt', type: 'int64', sort: true },
    { name: 'tags', type: 'string[]', optional: true, facet: true, infix: true },
  ],
...
export class ConstitutionScrapingService {
  ...
  private readonly _constitutionUrl = 'https://www.planalto.gov.br/ccivil_03/constituicao/ConstituicaoCompilado.htm';
    ...
    try {
      const response = await firstValueFrom(
        this._httpService.get(this._constitutionUrl, {
          ...
          transformResponse: [(data) => Buffer.from(data, 'binary').toString('latin1')],
        }),
      );
      this._logger.log('Successfully fetched HTML.');
      return response.data;
    } catch (error: any) {
      this._logger.error(`Error fetching constitution HTML: ${error.message}`, error.stack);
      throw error;
    }
  }
   ...
   */
  private _parseConstitutionHtml(htmlContent: string): RawConstitutionDataItem[] {
    ...
    $('body p').each((_idx, el) => {
      ...
      if (!text || text.length < 3 || /PRESIDÊNCIA DA REPÚBLICA|CASA CIVIL|Subchefia para Assuntos Jurídicos/i.test(text)) {
        return;
      }
      // Skip navigation links often found in headers/footers of such pages
      if (element.parents('table').length > 0 && element.find('a[href*="planalto.gov.br"]').length > 0 && element.text().length < 100) {
      ...
      if (/^EMENDA CONSTITUCIONAL Nº \d+/i.test(text) && (element.attr('align') === 'center' || element.find('b').length > 0)) {
        elementType = 'EMENDA CONSTITUCIONAL';
        // Reset context as it's a new top-level declaration
        currentContext = { title: text };
      }
      // ATO DAS DISPOSIÇÕES CONSTITUCIONAIS TRANSITÓRIAS
      else if (/^ATO DAS DISPOSIÇÕES CONSTITUCIONAIS TRANSITÓRIAS/i.test(text) && (element.attr('align') === 'center' || element.find('b').length > 0)) {
        elementType = 'ATO DAS DISPOSIÇÕES CONSTITUCIONAIS TRANSITÓRIAS';
        currentContext = { title: text }; // Reset for ADCT
      }
      // PREÂMBULO
      else if (/^PREÂMBULO/i.test(text) && (element.attr('align') === 'center' || element.find('b').length > 0)) {
        elementType = 'PREÂMBULO';
        currentContext = { title: text }; // Preâmbulo is its own context
      }
      // TÍTULO (Usually centered, bold, larger font)
      else if (text.startsWith('TÍTULO') && (element.attr('align') === 'center' || element.find('b').length > 0)) {
        elementType = 'TÍTULO';
        currentContext = { title: text, chapter: undefined, section: undefined, subSection: undefined, articleNumber: undefined };
      }
      // CAPÍTULO
      else if (text.startsWith('CAPÍTULO') && (element.attr('align') === 'center' || element.find('b').length > 0)) {
      ...
      // SEÇÃO
      else if (text.startsWith('Seção ') && (element.attr('align') === 'center' || element.find('b').length > 0)) { // Note: "Seção" not always all caps
        elementType = 'SEÇÃO';
       ...
       // SUBSEÇÃO
      else if (text.startsWith('Subseção ') && (element.attr('align') === 'center' || element.find('b').length > 0)) {
      ...
      // Parágrafo (§ Xº ...)
      else if (/^§\s*\d+º?\.?/i.test(text) || text.startsWith('Parágrafo único.')) {
        elementType = 'Parágrafo';
      }
      // Inciso (Roman numerals I, II, ..., or with text like "Inciso X -")
      else if (/^([IVXLCDM]+)\s*-\s*/.test(text) || text.toLowerCase().startsWith('inciso ')) {
        elementType = 'Inciso';
      }
      // Alínea (letters a), b) ...)
      ...
      else if (element.attr('align') === 'center' && text === text.toUpperCase() && text.split(' ').length >= 2 && text.split(' ').length <= 5 && $(el).nextAll('p:contains("Brasília")').length > 0) {
          elementType = 'ASSINATURA';
      }
      // Local e Data (Brasília, ...)
      else if (/^Brasília,\s*\d+\s*de\s*[a-zA-Z]+\s*de\s*\d{4}\./i.test(text) && element.attr('align') === 'center') {
          elementType = 'LOCAL_DATA';
      }
      // Texto Constitucional Promulgado (the final declaration)
      else if (/^Nós, representantes do povo brasileiro/i.test(text) || /A ASSEMBLEIA NACIONAL CONSTITUINTE/i.test(text)) {
      ...
      if (!elementType && text.length > 10) { // Avoid very short, possibly noise, texts
          if (currentContext.articleNumber) {
              elementType = 'Artigo'; // Or a more generic 'Content' type
          } else if (currentContext.title || currentContext.chapter || currentContext.section || currentContext.subSection) {
              ...
              if (currentContext.subSection) elementType = 'SUBSEÇÃO'; // CORRECTED CASE
              else if (currentContext.section) elementType = 'SEÇÃO';    // CORRECTED CASE
              else if (currentContext.chapter) elementType = 'CAPÍTULO';  // CORRECTED CASE
      ...
      } else if (text && text.length > 20) { // Log unclassified longer texts
          this._logger.debug(`Unclassified text (line ${lineNumber}, context: ${JSON.stringify(currentContext)}): ${text.substring(0,100)}...`);
      }
    });

    this._logger.log(`HTML parsing finished. Found ${items.length} raw items.`);
    if (items.length === 0) {
        this._logger.warn('Parser did not extract any items. Review selectors and HTML structure.');
    }
    return items;
  }
   ...
   */
  private _transformData(rawItems: RawConstitutionDataItem[]): ConstitutionIndexSchema[] {
    this._logger.log(`Transforming ${rawItems.length} raw items for Typesense...`);
    ...
    for (const item of rawItems) {
      let idSuffix = item.text.substring(0, 20).replace(/[^a-zA-Z0-9]/g, '') + '-' + item.lineNumber;
      let itemNumber: string | undefined;
      let typeForIndex: ConstitutionIndexSchema['type'] = item.elementType as ConstitutionIndexSchema['type']; // Initial cast
      ...
      if (item.elementType === 'TÍTULO') {
        ...
        currentFullReferenceParts = [item.hierarchicalContext.title || 'Desconhecido', item.text];
        lastArticleNumberForContext = undefined;
      } else if (item.elementType === 'SEÇÃO') {
        currentFullReferenceParts = [item.hierarchicalContext.title || 'Desconhecido', item.hierarchicalContext.chapter || 'Desconhecido', item.text];
        lastArticleNumberForContext = undefined;
      } else if (item.elementType === 'SUBSEÇÃO') {
        currentFullReferenceParts = [item.hierarchicalContext.title || 'Desconhecido', item.hierarchicalContext.chapter || 'Desconhecido', item.hierarchicalContext.section || 'Desconhecido', item.text];
        lastArticleNumberForContext = undefined;
      } else if (item.elementType === 'Artigo') {
        itemNumber = item.hierarchicalContext.articleNumber || item.text.match(/^Art\.\s*\d+º?\.?/i)?.[0];
        if (itemNumber) {
        ...
        itemNumber = item.text.match(/^([IVXLCDM]+)\s*-/i)?.[1] || item.text.match(/^Inciso\s+([IVXLCDM]+)/i)?.[1];
        ...
        typeForIndex = 'Alínea';
      } else if (item.elementType === 'PREÂMBULO' || item.elementType === 'TEXTO_CONSTITUCIONAL_PROMULGADO' || item.elementType === 'EMENDA CONSTITUCIONAL' || item.elementType === 'ATO DAS DISPOSIÇÕES CONSTITUCIONAIS TRANSITÓRIAS') {
          currentFullReferenceParts = [item.text];
          ...
          if(item.elementType === 'ATO DAS DISPOSIÇÕES CONSTITUCIONAIS TRANSITÓRIAS' && /^Art\.\s*\d+º?\.?/i.test(item.text)) {
          ...
          this._logger.debug(`Skipping transformation for item type: ${item.elementType} - Text: ${item.text.substring(0,50)}`);
          continue;
      }


      const fullReference = currentFullReferenceParts.filter(p => p && p.trim() !== '').join(', ');
      idSuffix = fullReference.replace(/[^a-zA-Z0-9]/g, '-') + '-' + item.lineNumber;
      ...
      const doc: ConstitutionIndexSchema = {
        ...
        hierarchicalTextContext: [
          ...
          item.elementType !== 'Artigo' && item.hierarchicalContext.articleNumber ?
            (rawItems.find(r => r.elementType === 'Artigo' && r.hierarchicalContext.articleNumber === item.hierarchicalContext.articleNumber)?.text || '')
            : ''
        ].filter(Boolean).join(' | ') + ` | ${item.text}`,
        ...
        parentArticleNumber: item.elementType !== 'Artigo' ? (item.hierarchicalContext.articleNumber || lastArticleNumberForContext) : undefined,
    ...
    this._logger.log(`Transformed ${documents.length} documents for Typesense.`);
    return documents;
  }

  private _extractTags(text: string, context: RawConstitutionDataItem['hierarchicalContext']): string[] {
    ...
    if (text.match(/direitos e garantias fundamentais/i)) tags.add('Direitos Fundamentais');
    ...
    if (text.match(/ordem econômica e financeira/i)) tags.add('Ordem Econômica');
  ...
  public async processConstitution(): Promise<void> {
    ...
    try {
      ...
      await this._typesenseService.ensureCollectionExists(CONSTITUTION_COLLECTION_NAME, CONSTITUTION_TYPESENSE_SCHEMA);
      ...
      if (documentsToIndex.length > 0) {
        await this._typesenseService.indexDocuments(CONSTITUTION_COLLECTION_NAME, documentsToIndex, 200); // Batch size 200
      } else {
      ...
      this._logger.error(`Critical error during constitution processing: ${error.message}`, error.stack);
    }
  }
}