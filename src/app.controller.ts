...
export class AppController {
  ...
  async triggerScraping(): Promise<{ message: string }> {
    ...
    this._appService
      .triggerConstitutionScrapingAndIndexing()
      .then((result) => this._logger.log(result.message))
      .catch((err) =>
        this._logger.error(
          'Async scraping process failed after request.',
          err.stack,
        ),
      );
    ...
    return {
      message:
        'Constitution scraping and indexing process has been initiated. Check server logs for progress and completion.',
    };
  }
