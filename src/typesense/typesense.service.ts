...
export class TypesenseService {
  ...
  constructor(@Inject(TYPESENSE_CLIENT) private readonly _typesenseClient: Client) {}
   ...
   */
  async ensureCollectionExists(name: string, schema: CollectionCreateSchema): Promise<void> {
    try {
      ...
      if (error.httpStatus === 404) {
        ...
        this._logger.error(`Error retrieving or creating collection "${name}": ${error.message}`, error.stack);
        throw error;
      }
    }
  }
  ...
  async indexDocuments<TDocument extends object>(
    ...
    if (!documents || documents.length === 0) {
      this._logger.warn(`No documents provided for indexing in "${collectionName}".`);
      return;
    }

    this._logger.log(`Preparing to index ${documents.length} documents into "${collectionName}" in batches of ${batchSize}.`);

    for (let i = 0; i < documents.length; i += batchSize) {
      const batch = documents.slice(i, i + batchSize);
      this._logger.log(`Indexing batch ${i / batchSize + 1} with ${batch.length} documents...`);
      try {
        const importResults = await this._typesenseClient
          .collections(collectionName)
          .documents()
          .import(batch, { action: 'upsert', batch_size: batch.length, dirty_values: 'coerce_or_drop' }); // Coerce or drop to handle potential type mismatches
        ...
        this._logger.error(
          ...
          error.importResults ? JSON.stringify(error.importResults, null, 2) : error.stack,
        );
        // Depending on the error, you might want to re-throw or handle it to continue with other batches
      }
    }
    this._logger.log(`Finished indexing all documents into "${collectionName}".`);
  }
  ...
  async deleteCollectionIfExists(collectionName: string): Promise<void> {
    try {
      ...
      if (error.httpStatus === 404) {
        this._logger.log(`Collection "${collectionName}" does not exist. No action taken.`);
      } else {
        this._logger.error(`Error deleting collection "${collectionName}": ${error.message}`, error.stack);
        throw error;
      }
    }
  }
   ...
   */
   async getCollectionSchema(collectionName: string): Promise<CollectionSchema> {
    try {
      const schema = await this._typesenseClient.collections(collectionName).retrieve();
      return schema;
    } catch (error: any) {
      this._logger.error(`Error retrieving schema for collection "${collectionName}": ${error.message}`, error.stack);
      throw error;
    }
  }
}